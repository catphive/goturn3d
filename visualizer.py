# Based on https://github.com/navoshta/KITTI-Dataset/blob/master/kitti-dataset.ipynb
...
def make_movie(data, tracklet_rects, tracklet_types):
    frames = []
    n_frames = len(list(data.velo))

    print('Preparing animation frames...')
    for i in range(n_frames):
        print_progress(i, n_frames - 1)
        filename = draw_3d_plot(i, data, tracklet_rects, tracklet_types)
        frames += [filename]
